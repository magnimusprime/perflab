...
#if IS_x86_64
/* $begin x8664cyclecounter */
/* Initialize the cycle counter */
static unsigned cyc_hi = 0;
static unsigned cyc_lo = 0;
static unsigned cyc_aux = 0;


/* Set *hi and *lo to the high and low order bits  of the cycle counter.  
   Implementation requires assembly code to use the rdtsc instruction. */
void access_counter_post(unsigned *hi, unsigned *lo, unsigned *aux)
{
    __asm__ volatile(
          "rdtscp" /* Read cycle counter */
	: "=d" (*hi), "=a" (*lo), "=c"(*aux)    /* and move results to */
	: /* No input */                        /* the two outputs */
	: "%rbx");
}

void access_counter_pre(unsigned *hi, unsigned *lo, unsigned *aux)
{
    __asm__ volatile(
          "cpuid; rdtscp" /* Read cycle counter */
	: "=d" (*hi), "=a" (*lo), "=c"(*aux)    /* and move results to */
	: /* No input */                        /* the two outputs */
	: "%rbx");
}
...
void start_counter()
{
    access_counter_pre(&cyc_hi, &cyc_lo, &cyc_aux);
}

/* Return the number of cycles since the last call to start_counter. */
double get_counter()
{
    unsigned ncyc_hi, ncyc_lo, ncyc_aux;
    unsigned hi, lo, borrow;
    double result;

    /* Get cycle counter */
    access_counter_post(&ncyc_hi, &ncyc_lo, &ncyc_aux);
...
static void callibrate(int verbose)
{
	...
	if (newt != INVALID_COUNTER && oldt != INVALID_COUNTER && newt-oldt >= THRESHOLD) {
	    clock_t newc;
	    times(&t);
	    newc = t.tms_utime;
	    if (newc > oldc) {
